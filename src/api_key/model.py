...
class ApiKeyModel(Base, TeamScopedMixin):
    ...
    삭제 정책: 회수(revoke) 시 ``is_active=False`` (BaseModel 의 공통 컬럼 재사용).
    회수 시점은 ``updated_at`` 으로 추적 (회수 후에는 row 수정을 허용하지 않음).
    """
    ...
    # 전체 토큰 ("tms_<token_urlsafe(32)>"). 인증 요청마다 조회되므로 인덱스 필수.
    key: Mapped[str] = mapped_column(String(255), nullable=False)
    # 키 앞 ~12자 — 목록 UI 에서 "tms_ab12…" 렌더링용 (비밀 부분 재조회 방지).
