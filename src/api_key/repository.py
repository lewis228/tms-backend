...
class ApiKeyRepository(TeamScopedRepoMixin):
    """team scoped API Key 레포.

    단 ``get_active_by_key`` 는 인증 진입점으로 호출 당시에는 team_id 를 모르므로,
    mixin 초기화 때 ``team_id=None`` 으로 생성한 뒤 **오직 이 한 메서드만** 사용한다.
    다른 메서드는 ``_require_team()`` 에서 실패시켜 오용을 막는다.
    """
    ...
    async def get_active_by_key(self, key: str) -> Optional[ApiKeyModel]:
        """인증 핫 패스 — team_id 를 **모르는 시점**에 키 문자열만으로 조회.
        생성 시 ``team_id=None`` 을 넘긴 리포 인스턴스에서만 사용 가능. 키가
        유효하면 해당 row 의 ``team_id`` 로 상위 인증 결과를 채운다.
    ...
    async def touch_last_used(self, api_key_id: int) -> None:
        """회수된 키여도 단순 기록. team 체크는 이미 auth 단계에서 완료된 후라 생략."""
