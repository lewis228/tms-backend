...
async def sqlalchemy_exception_handler(request: Request, exc: SQLAlchemyError):
    ...
    # MySQL 주요 코드 매핑
    # 1062: Duplicate entry (유니크 제약 위반) → 409
    # 1452: Cannot add or update a child row (FK 참조 실패) → 400
    # 1451: Cannot delete or update a parent row (FK RESTRICT) → 409
    # 1205: Lock wait timeout → 409
    # 1213: Deadlock found → 409
    # 3024/1317: 실행 시간 초과 / 쿼리 중단 → 504
    if code in (3024, 1317):
        ...
        err_code = "FK_REFERENCE_NOT_FOUND"
        message = "참조하는 리소스를 찾을 수 없습니다."
    elif code == 1451:
        http_status = status.HTTP_409_CONFLICT
        err_code = "FK_IN_USE"
        message = "다른 리소스가 참조 중이라 변경할 수 없습니다."
