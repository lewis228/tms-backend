...
async def create_customer(
    ...
    - 쓰기 권한 필요
    """
    return await CustomerService(db, team_id).create(
        body,
...
async def list_customers(
    ...
    """
    return await CustomerService(db, team_id).list_paginated(request)
...
async def sync_customers(
    ...
    """
    return await CustomerService(db, team_id).sync_delta(since)
...
async def get_customer(
    ...
    고객사 단건 조회(활성만)
    """
    return await CustomerService(db, team_id).get(customer_id)
...
async def update_customer(
    ...
    - 쓰기 권한 필요
    """
    return await CustomerService(db, team_id).update(
        customer_id,
        body,
        actor_user_id=int(me.id),
    )
...
async def delete_customer(
    ...
    """
    return await CustomerService(db, team_id).delete(
        customer_id,
        actor_user_id=int(me.id),
    )
...
async def create_customers_bulk(
    ...
    - 부분 성공 허용
    """
    return await CustomerService(db, team_id).create_bulk(
        body,
        actor_user_id=int(me.id),
    )
...
async def update_customers_bulk(
    ...
    - 존재하지 않는 ID는 실패 처리
    """
    return await CustomerService(db, team_id).update_bulk(
        body,
        actor_user_id=int(me.id),
    )
...
async def delete_customers_bulk(
    ...
    """
    return await CustomerService(db, team_id).delete_bulk(
        body,
        actor_user_id=int(me.id),
    )