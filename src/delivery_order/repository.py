# src/delivery_order/repository.py
...
from delivery_order.schemas.request import PaginateDeliveryOrderRequest
from delivery_order.schemas.response import DeliveryOrderResponseSchema


class DeliveryOrderRepository(TenantScopedRepoMixin):
    """
    DeliveryOrder(거래처) 리포지토리
    ...
    async def get(self, delivery_order_id: int) -> Optional[DeliveryOrderModel]:
        """
        단건 조회(활성만) + 최소 컬럼 로딩
        """
        q = (
            ...
            .where(
                DeliveryOrderModel.tenant_id == self._require_tenant(),
    ...
    async def get_many(self, delivery_order_ids: List[int]) -> List[DeliveryOrderModel]:
        """
        여러 건 조회 (활성만)
    ...
    async def get_paginated(self, request: PaginateDeliveryOrderRequest) -> CursorPaginationResult[DeliveryOrderResponseSchema]:
        """
        ...
        base_conditions = [DeliveryOrderModel.tenant_id == tenant_id]
        if not request.include_inactive:
            base_conditions.append(DeliveryOrderModel.is_active.is_(True))
    ...
    async def update(
        ...
        """
        if not payload:
            return await self.get(delivery_order_id)
    ...
    async def hard_delete_by_id(self, delivery_order_id: int) -> None:
        """단건 하드 삭제"""
        await self.db.execute(
            delete(DeliveryOrderModel).where(
                DeliveryOrderModel.tenant_id == self._require_tenant(),
                DeliveryOrderModel.id == delivery_order_id,
            )
        )
    ...
    async def get_existing_active_ids(self, ids: Iterable[int]) -> set[int]:
        ...
        stmt = (
            select(DeliveryOrderModel.id)
            .where(
                DeliveryOrderModel.tenant_id == self._require_tenant(),
                DeliveryOrderModel.is_active.is_(True),
                DeliveryOrderModel.id.in_(id_list),
            )
        )
    ...
    async def sync_delta(self, since):
        ...
        base_query = (
            select(DeliveryOrderModel)
            .where(DeliveryOrderModel.tenant_id == tenant_id)
            .options(
                load_only(
                    DeliveryOrderModel.id,
                    DeliveryOrderModel.name,
