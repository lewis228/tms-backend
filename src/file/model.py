...
class FileAssetModel(Base):
    ...
    - Product, Partner 등: tenant_id 있음 → 팀 삭제 시 CASCADE
    - User: tenant_id=NULL → 플랫폼 전역, User 삭제 시 명시적 삭제
    
    경로 구조:
    - tenant_id 있음: private/tenant-{id}/{domain}/{object_id}/...
    - tenant_id=NULL: private/{domain}/{object_id}/...
    
    삭제 정책:
    - 팀 하드 삭제 시: tenant_id FK(CASCADE)로 해당 팀 파일만 자동 삭제
    - User 삭제 시: 코드에서 명시적으로 delete_scope() 호출
    """
    __tablename__ = "file_asset"

    #  tenant_id 직접 정의 (nullable=True)
    ...
    # 논리 경로(브라우저 접근 기준)
    # 예: "private/tenant-123/product/45/images/a.jpg" (팀 소속)
    # 예: "private/user/1/profile/photo.jpg" (플랫폼 전역)
    logical_path:  Mapped[str] = mapped_column(String(512), nullable=False, index=True)

    __table_args__ = (
        # 팀 스코프 유니크 (tenant_id=NULL인 경우도 포함)
        UniqueConstraint("tenant_id", "id", name="uq_file_asset_tenant_id_id"),
        # 조회 최적화
        Index("ix_file_asset_tenant_id_id", "tenant_id", "id"),
        Index("ix_file_asset_team_domain_obj", "tenant_id", "domain", "object_id"),
        Index("ix_file_asset_team_subdir", "tenant_id", "subdir"),
        Index("ix_file_asset_logical_path", "logical_path"),
        #  User 도메인용 인덱스 (tenant_id=NULL)
