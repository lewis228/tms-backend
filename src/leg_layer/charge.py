...
"""Add-on 기본 단가 산출 — accessorial 마스터(code + driver override)에서.

컨플루언스 재정의: leg 의 추가요금은 모두 Add-on. 생성 시 시스템이 기본 금액을 채우고,
사용자가 수정/삭제 가능. 단가 출처 = accessorial 마스터(code 일치, driver override 우선).
"""
...
from accessorial.const.status import AccessorialUnit
...
async def resolve_addon_amount(
    ...
    accessorial 정의 없으면 None.
    """
    acc = AccessorialRepository(db, team_id)
    rule = await acc.find_for_code(code, driver_id)
    if rule is None:
        return None
    if rule.unit == AccessorialUnit.PERCENT and rule.percent is not None:
        if base_amount is None:
            return None
        return ((base_amount * rule.percent).quantize(_Q), None, Decimal("1"))
    if rule.unit == AccessorialUnit.MILE and rule.amount is not None and rate_miles is not None:
