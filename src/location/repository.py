...
from location.schemas.request import PaginateLocationRequest
...
class LocationRepository(TeamScopedRepoMixin):
    ...
    async def get(self, location_id: int) -> Optional[LocationModel]:
        """
        단건 조회(활성만) + 최소 컬럼 로딩
        """
        q = (
    ...
    async def get_many(self, location_ids: List[int]) -> List[LocationModel]:
        """
        여러 건 조회 (활성만)
    ...
    async def get_paginated(self, request: PaginateLocationRequest) -> CursorPaginationResult[LocationResponseSchema]:
        """
    ...
    async def hard_delete_by_id(self, location_id: int) -> None:
        """단건 하드 삭제"""
