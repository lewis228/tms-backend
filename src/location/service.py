...
class LocationService:
    ...
    async def create(
        ...
        return LocationResponseSchema.model_validate(row)
    ...
    async def create_bulk(
        self,
        payload: LocationBulkCreateRequest,
        actor_user_id: int | None = None,
    ) -> LocationBulkCreateResponseSchema:
        """
        ...
        for item in payload.items:
            ...
            location = LocationResponseSchema.model_validate(row)
        ...
        return LocationBulkCreateResponseSchema(
            results=results,
            summary=BulkSummary(
    ...
    async def get(self, location_id: int) -> LocationResponseSchema:
        row = await self.repo.get(location_id)
        if not row:
            raise NotFoundException("거래처")
        return LocationResponseSchema.model_validate(row)

    async def list_paginated(
        self, request: PaginateLocationRequest
    ) -> CursorPaginationResult[LocationResponseSchema]:
        """
        커서 기반 페이지네이션:
          - meta.count / meta.hasMore / data(LocationResponseSchema[])
        """
        result = await self.repo.get_paginated(request)
        result.data = [LocationResponseSchema.model_validate(r) for r in result.data]
        return result
    ...
    async def sync_delta(self, since_str: str):
        ...
        result.items = [
            LocationResponseSchema.model_validate(r)
    ...
    async def update_bulk(
        self,
        payload: LocationBulkUpdateRequest,
        actor_user_id: int | None = None,
    ) -> LocationBulkUpdateResponseSchema:
        """
        ...
        # 여기까지 오면 전부 성공
        return LocationBulkUpdateResponseSchema(
            results=results,
            summary=BulkSummary(
                total=len(payload.items),
                succeeded=len(results),
    ...
    async def delete(
        ...
        return LocationDeleteResponseSchema(
            id=location_id,
            deleted=True,
            soft_deleted=True,
        )
    ...
    async def delete_bulk(
        self,
        payload: LocationBulkDeleteRequest,
        actor_user_id: int | None = None,
    ) -> LocationBulkDeleteResponseSchema:
        """
        거래처 벌크 삭제 (항상 소프트 삭제)
        ...
        # 2. 전체 소프트 삭제
        for location_id in payload.ids:
            await self.repo.soft_deactivate_by_id(
                location_id,
                actor_user_id=actor_user_id,
            )
        ...
        return LocationBulkDeleteResponseSchema(
            results=results,
            summary=BulkSummary(
