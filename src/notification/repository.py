# src/notification/repository.py
...
from notification.schemas.request import PaginateNotificationRequest
...
class NotificationRepository(TenantScopedRepoMixin):
    ...
    async def get(self, notification_id: int) -> Optional[NotificationModel]:
        """
        단건 조회(활성만) + 최소 컬럼 로딩
        """
        q = (
            ...
            .where(
                NotificationModel.tenant_id == self._require_tenant(),
    ...
    async def get_many(self, notification_ids: List[int]) -> List[NotificationModel]:
        """
        여러 건 조회 (활성만)
    ...
    async def get_paginated(self, request: PaginateNotificationRequest) -> CursorPaginationResult[NotificationResponseSchema]:
        """
        ...
        base_conditions = [NotificationModel.tenant_id == tenant_id]
        if not request.include_inactive:
            base_conditions.append(NotificationModel.is_active.is_(True))
    ...
    async def hard_delete_by_id(self, notification_id: int) -> None:
        """단건 하드 삭제"""
    ...
    async def get_existing_active_ids(self, ids: Iterable[int]) -> set[int]:
        ...
        stmt = (
            ...
            .where(
                NotificationModel.tenant_id == self._require_tenant(),
                NotificationModel.is_active.is_(True),
                NotificationModel.id.in_(id_list),
            )
        )
