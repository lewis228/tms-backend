...
async def create_notification(
    ...
    - 쓰기 권한 필요
    """
    return await NotificationService(db, team_id).create(
        body,
...
async def list_notifications(
    ...
    """
    return await NotificationService(db, team_id).list_paginated(request)
...
async def sync_notifications(
    ...
    """
    return await NotificationService(db, team_id).sync_delta(since)
...
async def get_notification(
    ...
    거래처 단건 조회(활성만)
    """
    return await NotificationService(db, team_id).get(notification_id)
...
async def update_notification(
    ...
    - 쓰기 권한 필요
    """
    return await NotificationService(db, team_id).update(
        notification_id,
        body,
        actor_user_id=int(me.id),
    )
...
async def delete_notification(
    ...
    """
    return await NotificationService(db, team_id).delete(
        notification_id,
        actor_user_id=int(me.id),
    )
...
async def create_notifications_bulk(
    ...
    - 부분 성공 허용
    """
    return await NotificationService(db, team_id).create_bulk(
        body,
        actor_user_id=int(me.id),
    )
...
async def update_notifications_bulk(
    ...
    - 존재하지 않는 ID는 실패 처리
    """
    return await NotificationService(db, team_id).update_bulk(
        body,
        actor_user_id=int(me.id),
    )
...
async def delete_notifications_bulk(
    ...
    """
    return await NotificationService(db, team_id).delete_bulk(
        body,
        actor_user_id=int(me.id),
    )