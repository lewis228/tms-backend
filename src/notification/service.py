...
class NotificationService:
    ...
    def __init__(self, db: AsyncSession, tenant_id: int):
        self.db = db
        self.repo = NotificationRepository(db, tenant_id)
    ...
    async def create(
        ...
        return NotificationResponseSchema.model_validate(row)
    ...
    async def create_bulk(
        ...
        actor_user_id: int | None = None,
    ) -> NotificationBulkCreateResponseSchema:
        """
        ...
        for item in payload.items:
            ...
            notification = NotificationResponseSchema.model_validate(row)
        ...
        return NotificationBulkCreateResponseSchema(
            results=results,
            summary=BulkSummary(
    ...
    async def get(self, notification_id: int) -> NotificationResponseSchema:
        row = await self.repo.get(notification_id)
        if not row:
            raise NotFoundException("거래처")
        return NotificationResponseSchema.model_validate(row)

    async def list_paginated(
        self, request: PaginateNotificationRequest
    ) -> CursorPaginationResult[NotificationResponseSchema]:
        """
        커서 기반 페이지네이션:
          - meta.count / meta.hasMore / data(NotificationResponseSchema[])
        """
        result = await self.repo.get_paginated(request)
        result.data = [NotificationResponseSchema.model_validate(r) for r in result.data]
        return result
    ...
    async def sync_delta(self, since_str: str):
        ...
        result.items = [
            NotificationResponseSchema.model_validate(r)
    ...
    async def update_bulk(
        self,
        payload: NotificationBulkUpdateRequest,
        actor_user_id: int | None = None,
    ) -> NotificationBulkUpdateResponseSchema:
        """
        ...
        # 여기까지 오면 전부 성공
        return NotificationBulkUpdateResponseSchema(
            results=results,
            summary=BulkSummary(
                total=len(payload.items),
                succeeded=len(results),
    ...
    async def delete(
        ...
        return NotificationDeleteResponseSchema(
            id=notification_id,
            deleted=True,
            soft_deleted=True,
        )
    ...
    async def delete_bulk(
        ...
        actor_user_id: int | None = None,
    ) -> NotificationBulkDeleteResponseSchema:
        """
        거래처 벌크 삭제 (항상 소프트 삭제)
        ...
        return NotificationBulkDeleteResponseSchema(
            results=results,
            summary=BulkSummary(
