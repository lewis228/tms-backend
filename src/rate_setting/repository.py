# src/rate_setting/repository.py
...
from rate_setting.schemas.request import PaginateRateSettingRequest
from rate_setting.schemas.response import RateSettingResponseSchema


class RateSettingRepository(TenantScopedRepoMixin):
    """
    RateSetting(거래처) 리포지토리
    ...
    async def get(self, rate_setting_id: int) -> Optional[RateSettingModel]:
        """
        단건 조회(활성만) + 최소 컬럼 로딩
        """
        q = (
            ...
            .where(
                RateSettingModel.tenant_id == self._require_tenant(),
    ...
    async def get_many(self, rate_setting_ids: List[int]) -> List[RateSettingModel]:
        """
        여러 건 조회 (활성만)
        - 순서 보장 안 됨, 없는 ID는 결과에서 제외
        """
        if not rate_setting_ids:
            return []
    ...
    async def get_paginated(self, request: PaginateRateSettingRequest) -> CursorPaginationResult[RateSettingResponseSchema]:
        """
        ...
        base_conditions = [RateSettingModel.tenant_id == tenant_id]
        if not request.include_inactive:
            base_conditions.append(RateSettingModel.is_active.is_(True))
    ...
    async def update(
        ...
        """
        if not payload:
            return await self.get(rate_setting_id)
    ...
    async def hard_delete_by_id(self, rate_setting_id: int) -> None:
        """단건 하드 삭제"""
        await self.db.execute(
            delete(RateSettingModel).where(
                RateSettingModel.tenant_id == self._require_tenant(),
                RateSettingModel.id == rate_setting_id,
            )
        )
    ...
    async def get_existing_active_ids(self, ids: Iterable[int]) -> set[int]:
        ...
        stmt = (
            select(RateSettingModel.id)
            .where(
                RateSettingModel.tenant_id == self._require_tenant(),
                RateSettingModel.is_active.is_(True),
                RateSettingModel.id.in_(id_list),
            )
        )
    ...
    async def sync_delta(self, since):
        ...
        base_query = (
            select(RateSettingModel)
            .where(RateSettingModel.tenant_id == tenant_id)
            .options(
                load_only(
                    RateSettingModel.id,
                    RateSettingModel.name,
