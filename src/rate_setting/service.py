...
from rate_setting.repository import RateSettingRepository
...
class RateSettingService:
    ...
    def __init__(self, db: AsyncSession, tenant_id: int):
        self.db = db
        self.repo = RateSettingRepository(db, tenant_id)
    ...
    async def create(
        self,
        payload: RateSettingCreateRequest,
        actor_user_id: int | None = None,
    ) -> RateSettingResponseSchema:
        row = await self.repo.create(
            payload.model_dump(),
            actor_user_id=actor_user_id,
        )
        return RateSettingResponseSchema.model_validate(row)
    ...
    async def create_bulk(
        self,
        payload: RateSettingBulkCreateRequest,
        actor_user_id: int | None = None,
    ) -> RateSettingBulkCreateResponseSchema:
        """
        ...
        for item in payload.items:
            ...
            rate_setting = RateSettingResponseSchema.model_validate(row)
        ...
        return RateSettingBulkCreateResponseSchema(
            results=results,
            summary=BulkSummary(
    ...
    async def get(self, rate_setting_id: int) -> RateSettingResponseSchema:
        row = await self.repo.get(rate_setting_id)
        if not row:
            raise NotFoundException("거래처")
        return RateSettingResponseSchema.model_validate(row)

    async def list_paginated(
        self, request: PaginateRateSettingRequest
    ) -> CursorPaginationResult[RateSettingResponseSchema]:
        """
        커서 기반 페이지네이션:
          - meta.count / meta.hasMore / data(RateSettingResponseSchema[])
        """
        result = await self.repo.get_paginated(request)
        result.data = [RateSettingResponseSchema.model_validate(r) for r in result.data]
        return result
    ...
    async def sync_delta(self, since_str: str):
        ...
        result.items = [
            RateSettingResponseSchema.model_validate(r)
    ...
    async def update(
        self,
        rate_setting_id: int,
        payload: RateSettingUpdateRequest,
        actor_user_id: int | None = None,
    ) -> RateSettingResponseSchema:
    ...
    async def update_bulk(
        self,
        payload: RateSettingBulkUpdateRequest,
        actor_user_id: int | None = None,
    ) -> RateSettingBulkUpdateResponseSchema:
        """
        ...
        # 여기까지 오면 전부 성공
        return RateSettingBulkUpdateResponseSchema(
            results=results,
            summary=BulkSummary(
                total=len(payload.items),
                succeeded=len(results),
    ...
    async def delete(
        ...
        rate_setting_id: int,
        actor_user_id: int | None = None,
    ) -> RateSettingDeleteResponseSchema:
        """
        거래처 삭제 (항상 소프트 삭제)
        ...
        return RateSettingDeleteResponseSchema(
            id=rate_setting_id,
            deleted=True,
            soft_deleted=True,
        )
    ...
    async def delete_bulk(
        self,
        payload: RateSettingBulkDeleteRequest,
        actor_user_id: int | None = None,
    ) -> RateSettingBulkDeleteResponseSchema:
        """
        거래처 벌크 삭제 (항상 소프트 삭제)
        ...
        # 2. 전체 소프트 삭제
        for rate_setting_id in payload.ids:
            await self.repo.soft_deactivate_by_id(
                rate_setting_id,
                actor_user_id=actor_user_id,
            )
        ...
        return RateSettingBulkDeleteResponseSchema(
            results=results,
            summary=BulkSummary(
