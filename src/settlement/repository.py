...
from settlement.schemas.request import PaginateSettlementRequest
...
class SettlementRepository(TeamScopedRepoMixin):
    ...
    async def get(self, settlement_id: int) -> Optional[SettlementModel]:
        """
        단건 조회(활성만) + 최소 컬럼 로딩
        """
        q = (
    ...
    async def get_many(self, settlement_ids: List[int]) -> List[SettlementModel]:
        """
        여러 건 조회 (활성만)
    ...
    async def get_paginated(self, request: PaginateSettlementRequest) -> CursorPaginationResult[SettlementResponseSchema]:
        """
    ...
    async def hard_delete_by_id(self, settlement_id: int) -> None:
        """단건 하드 삭제"""
