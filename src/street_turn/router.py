...
async def create_street_turn(
    ...
    - 쓰기 권한 필요
    """
    return await StreetTurnService(db, tenant_id).create(
        body,
...
async def list_street_turns(
    ...
    """
    return await StreetTurnService(db, tenant_id).list_paginated(request)
...
async def sync_street_turns(
    ...
    """
    return await StreetTurnService(db, tenant_id).sync_delta(since)
...
async def get_street_turn(
    ...
    거래처 단건 조회(활성만)
    """
    return await StreetTurnService(db, tenant_id).get(street_turn_id)
...
async def update_street_turn(
    ...
    - 쓰기 권한 필요
    """
    return await StreetTurnService(db, tenant_id).update(
        street_turn_id,
        body,
        actor_user_id=int(me.id),
    )
...
async def delete_street_turn(
    ...
    """
    return await StreetTurnService(db, tenant_id).delete(
        street_turn_id,
        actor_user_id=int(me.id),
    )
...
async def create_street_turns_bulk(
    ...
    - 부분 성공 허용
    """
    return await StreetTurnService(db, tenant_id).create_bulk(
        body,
        actor_user_id=int(me.id),
    )
...
async def update_street_turns_bulk(
    ...
    - 존재하지 않는 ID는 실패 처리
    """
    return await StreetTurnService(db, tenant_id).update_bulk(
        body,
        actor_user_id=int(me.id),
    )
...
async def delete_street_turns_bulk(
    ...
    """
    return await StreetTurnService(db, tenant_id).delete_bulk(
        body,
        actor_user_id=int(me.id),
    )