...
class TenantRepository:
    ...
    # 1) Response Schema (TenantListItemResponseSchema)
    ...
    # 3) Service
    #    - 수동 변환은 지양합니다.
    #      result.data = [TenantListItemResponseSchema.model_validate(r) for r in result.data]
    #      같은 직변환 패턴을 유지하세요.
    #
    # [주의: Lazy 정책]
    ...
    #
    # [빠른 적용 예시]
    # - 스키마: TenantListItemResponseSchema에서 불필요 필드 제거
    ...
    def _with_options_detail(self, stmt):
        return stmt.options(
            load_only(
                ...
                TenantModel.image_url,
                TenantModel.company_name,
                TenantModel.registration_number,
                TenantModel.address,
                TenantModel.representative_name,
                TenantModel.phone_number,
                TenantModel.currency,
                TenantModel.decimal_places,
                TenantModel.product_info_display,
                TenantModel.product_info_template,
                TenantModel.excel_product_identification,
                TenantModel.gs1_gtin_enabled,
            ),
            # Team → members (활성만)
            selectinload(TenantModel.members)
                .load_only(
                    UserTenantModel.id,
            ...
            # Team → files (이미지 등)
            selectinload(TenantModel.files),
        )
    ...
    # ─────────────────────────────────────────────
    #
    # - UserTenantModel 자체의 최소 컬럼 + 중첩 user/permission_group의 최소 컬럼만 로드
    # - 서비스는 ORM을 그대로 UserTenantResponseSchema로 직변환(from_attributes=True)
    ...
    def _with_options_members(self, stmt):
        return stmt.options(
            ...
            selectinload(UserTenantModel.user).load_only(
                UserModel.id,
                UserModel.email,
                UserModel.role,
                UserModel.name,
            ...
            selectinload(UserTenantModel.permission_group).load_only(
                PermissionGroupModel.id,
                PermissionGroupModel.name,
            ),
        )

    # tenant_id로 활성 팀 단건 조회(활성 멤버 로딩 포함)
    async def get_tenant(self, tenant_id: int) -> Optional[TenantModel]:
        """활성 팀 단일 조회"""
        stmt = (
            select(TenantModel)
            .where(TenantModel.id == tenant_id, TenantModel.is_active.is_(True))
        )
        stmt = self._with_options_detail(stmt)
        return await self.db.scalar(stmt)

    # tenant_id로 비활성 포함 팀 단건 조회(삭제/복구 등의 내부용)
    async def get_tenant_including_inactive(self, tenant_id: int) -> Optional[TenantModel]:
    ...
    # 팀 이름 변경(UPDATE)
    async def rename_tenant(self, tenant_id: int, *, name: str) -> None:
        await self.db.execute(
            update(TenantModel).where(TenantModel.id == tenant_id).values(name=name)
        )

    # 팀 하드 삭제 — 모든 팀 스코프 데이터 물리 삭제
    async def hard_delete_tenant(self, tenant_id: int) -> None:
        """물리 삭제 — 팀 스코프 데이터 전체 삭제

        tenant_id FK(CASCADE)만으로는 도메인 테이블 간 composite FK(RESTRICT)가
        ...
        별도 수정이 필요 없다.
        """
        from common.model.tenant_scoped_mixin import get_tenant_scoped_table_names

        await self.db.execute(text("SET FOREIGN_KEY_CHECKS = 0"))
        try:
            for tbl in get_tenant_scoped_table_names():
                await self.db.execute(
    ...
    async def fetch_user_tenants(self, user_id: int) -> List[TenantModel]:
        """
        사용자 소속 + 활성 팀 목록
        ...
        stmt = (
            select(TenantModel)
            .join(UserTenantModel, UserTenantModel.tenant_id == TenantModel.id)
            .where(UserTenantModel.user_id == user_id, TenantModel.is_active.is_(True))
        )
        stmt = self._with_options_list(stmt).order_by(TenantModel.name.asc())
        res = await self.db.execute(stmt)
        return list(res.scalars().all())

    # 팀 멤버 수 집계(count)
    async def count_tenant_members(self, tenant_id: int) -> int:
        q = select(func.count(UserTenantModel.user_id)).where(UserTenantModel.tenant_id == tenant_id)
        return (await self.db.execute(q)).scalar_one()

    # 팀 내 관리자 그룹 멤버 수
    async def count_admin_members(self, tenant_id: int) -> int:
        stmt = (
            select(func.count(UserTenantModel.user_id))
            .select_from(UserTenantModel)
            .join(PermissionGroupModel, PermissionGroupModel.id == UserTenantModel.permission_group_id)
            .where(
    ...
    async def is_admin_member(self, tenant_id: int, user_id: int) -> bool:
        stmt = (
            select(func.count())
            .select_from(UserTenantModel)
            .join(PermissionGroupModel, PermissionGroupModel.id == UserTenantModel.permission_group_id)
    ...
    async def is_member(self, tenant_id: int, user_id: int) -> bool:
        stmt = select(func.count()).select_from(UserTenantModel).where(
            UserTenantModel.tenant_id == tenant_id,
            UserTenantModel.user_id == user_id,
        )
        return (await self.db.execute(stmt)).scalar_one() > 0
    ...
    async def add_member(self, *, tenant_id: int, user_id: int, permission_group_id: Optional[int]) -> None:
        self.db.add(UserTenantModel(
            user_id=user_id,
            tenant_id=tenant_id,
    ...
    async def get_active_member_user_ids(self, tenant_id: int) -> list[int]:
        """팀의 활성 멤버 user_id 목록 조회."""
        rows = await self.db.execute(
            select(UserTenantModel.user_id).where(
                UserTenantModel.tenant_id == tenant_id,
                UserTenantModel.is_active.is_(True),
            )
        )
    ...
    # ── 팀 설정 업데이트 ─────────────────────────────────────────────
    async def update_settings(self, tenant_id: int, update_data: dict) -> None:
        """전달된 필드만 UPDATE"""
        if not update_data:
            return
        await self.db.execute(
    ...
    # ── 팀 사용량 통계 ───────────────────────────────────────────────
    async def get_usage_stats(self, tenant_id: int) -> dict:
        ...
        member_count = (await self.db.execute(
            select(func.count(UserTenantModel.id)).where(
                UserTenantModel.tenant_id == tenant_id,
                UserTenantModel.is_active.is_(True),
            )
        )).scalar_one()
    ...
    async def add_permissions_to_group(
        self, *, tenant_id: int, group_id: int, permission_ids: Iterable[int]
    ) -> None:
        """
        팀 스코프 테이블(permission_group_permissions)에 INSERT 시
        tenant_id가 반드시 필요함. (NOT NULL + (tenant_id, group_id) FK)
        """
        grp = await self.db.get(PermissionGroupModel, group_id)
        if not grp:
            raise ValueError(f"group not found: {group_id}")
        if grp.tenant_id != tenant_id:
            raise ValueError(f"tenant mismatch: group.tenant_id={grp.tenant_id} vs arg tenant_id={tenant_id}")
    ...
    ) -> CursorPaginationResult[TenantModel]:
        base = (
            select(TenantModel)
            .join(UserTenantModel, UserTenantModel.tenant_id == TenantModel.id)
            .where(
    ...
    async def list_members_paginated(
        ...
        *,
    ...
    ) -> CursorPaginationResult[UserTenantModel]:
        """
        팀 멤버(유저-팀 조인) 목록을 커서 페이지네이션으로 반환.
        - base 쿼리에서 user_tenants.is_active=True만 선택
        - 관계 로딩은 selectinload + load_only로 최소 컬럼 명시
        - 여기서는 ORM 그대로 반환(가공 X) → 서비스에서 UserTenantResponseSchema로 직변환
        """
        ...
        result = await common.paginate(
            ...
            session=self.db,
            base_query=base,
            path=f"tenant/{tenant_id}/members",
        )
        return result

    # ── Delta Sync: 멤버 목록 (hard-delete → all_ids) ──────────────
    async def sync_members_delta(self, tenant_id: int, since: datetime):
        """
