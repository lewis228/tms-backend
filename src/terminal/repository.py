...
from terminal.schemas.request import PaginateTerminalRequest
from terminal.schemas.response import TerminalResponseSchema


class TerminalRepository(TenantScopedRepoMixin):
    """
    Terminal(거래처) 리포지토리
    ...
    async def get(self, terminal_id: int) -> Optional[TerminalModel]:
        """
        단건 조회(활성만) + 최소 컬럼 로딩
        """
        q = (
            ...
            .where(
                TerminalModel.tenant_id == self._require_tenant(),
    ...
    async def get_many(self, terminal_ids: List[int]) -> List[TerminalModel]:
        """
        여러 건 조회 (활성만)
    ...
    async def get_paginated(self, request: PaginateTerminalRequest) -> CursorPaginationResult[TerminalResponseSchema]:
        """
        ...
        base_conditions = [TerminalModel.tenant_id == tenant_id]
        if not request.include_inactive:
            base_conditions.append(TerminalModel.is_active.is_(True))
    ...
    async def hard_delete_by_id(self, terminal_id: int) -> None:
        """단건 하드 삭제"""
        await self.db.execute(
            delete(TerminalModel).where(
                TerminalModel.tenant_id == self._require_tenant(),
                TerminalModel.id == terminal_id,
            )
        )
    ...
    async def get_existing_active_ids(self, ids: Iterable[int]) -> set[int]:
        ...
        stmt = (
            select(TerminalModel.id)
            .where(
                TerminalModel.tenant_id == self._require_tenant(),
                TerminalModel.is_active.is_(True),
                TerminalModel.id.in_(id_list),
            )
        )
    ...
    async def sync_delta(self, since):
        ...
        base_query = (
            select(TerminalModel)
            .where(TerminalModel.tenant_id == tenant_id)
            .options(
                load_only(
                    TerminalModel.id,
                    TerminalModel.name,
