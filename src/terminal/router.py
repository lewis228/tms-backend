...
async def create_terminal(
    ...
    - 쓰기 권한 필요
    """
    return await TerminalService(db, team_id).create(
        body,
...
async def list_terminals(
    ...
    """
    return await TerminalService(db, team_id).list_paginated(request)
...
async def sync_terminals(
    ...
    """
    return await TerminalService(db, team_id).sync_delta(since)
...
async def get_terminal(
    ...
    거래처 단건 조회(활성만)
    """
    return await TerminalService(db, team_id).get(terminal_id)
...
async def update_terminal(
    ...
    - 쓰기 권한 필요
    """
    return await TerminalService(db, team_id).update(
        terminal_id,
        body,
        actor_user_id=int(me.id),
    )
...
async def delete_terminal(
    ...
    """
    return await TerminalService(db, team_id).delete(
        terminal_id,
        actor_user_id=int(me.id),
    )
...
async def create_terminals_bulk(
    ...
    - 부분 성공 허용
    """
    return await TerminalService(db, team_id).create_bulk(
        body,
        actor_user_id=int(me.id),
    )
...
async def update_terminals_bulk(
    ...
    - 존재하지 않는 ID는 실패 처리
    """
    return await TerminalService(db, team_id).update_bulk(
        body,
        actor_user_id=int(me.id),
    )
...
async def delete_terminals_bulk(
    ...
    """
    return await TerminalService(db, team_id).delete_bulk(
        body,
        actor_user_id=int(me.id),
    )