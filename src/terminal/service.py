...
from terminal.repository import TerminalRepository
...
class TerminalService:
    ...
    def __init__(self, db: AsyncSession, tenant_id: int):
        self.db = db
        self.repo = TerminalRepository(db, tenant_id)
    ...
    async def create(
        ...
        return TerminalResponseSchema.model_validate(row)
    ...
    async def create_bulk(
        self,
        payload: TerminalBulkCreateRequest,
        actor_user_id: int | None = None,
    ) -> TerminalBulkCreateResponseSchema:
        """
        ...
        for item in payload.items:
            ...
            terminal = TerminalResponseSchema.model_validate(row)
        ...
        return TerminalBulkCreateResponseSchema(
            results=results,
            summary=BulkSummary(
    ...
    async def get(self, terminal_id: int) -> TerminalResponseSchema:
        row = await self.repo.get(terminal_id)
        if not row:
            raise NotFoundException("거래처")
        return TerminalResponseSchema.model_validate(row)

    async def list_paginated(
        self, request: PaginateTerminalRequest
    ) -> CursorPaginationResult[TerminalResponseSchema]:
        """
        커서 기반 페이지네이션:
          - meta.count / meta.hasMore / data(TerminalResponseSchema[])
        """
        result = await self.repo.get_paginated(request)
        result.data = [TerminalResponseSchema.model_validate(r) for r in result.data]
        return result
    ...
    async def sync_delta(self, since_str: str):
        ...
        result.items = [
            TerminalResponseSchema.model_validate(r)
    ...
    async def update_bulk(
        self,
        payload: TerminalBulkUpdateRequest,
        actor_user_id: int | None = None,
    ) -> TerminalBulkUpdateResponseSchema:
        """
        ...
        # 여기까지 오면 전부 성공
        return TerminalBulkUpdateResponseSchema(
            results=results,
            summary=BulkSummary(
                total=len(payload.items),
                succeeded=len(results),
    ...
    async def delete(
        ...
        return TerminalDeleteResponseSchema(
            id=terminal_id,
            deleted=True,
            soft_deleted=True,
        )
    ...
    async def delete_bulk(
        self,
        payload: TerminalBulkDeleteRequest,
        actor_user_id: int | None = None,
    ) -> TerminalBulkDeleteResponseSchema:
        """
        거래처 벌크 삭제 (항상 소프트 삭제)
        ...
        # 2. 전체 소프트 삭제
        for terminal_id in payload.ids:
            await self.repo.soft_deactivate_by_id(
                terminal_id,
                actor_user_id=actor_user_id,
            )
        ...
        return TerminalBulkDeleteResponseSchema(
            results=results,
            summary=BulkSummary(
