...
from vessel.schemas.request import PaginateVesselRequest
...
class VesselRepository(TeamScopedRepoMixin):
    ...
    async def get(self, vessel_id: int) -> Optional[VesselModel]:
        """
        단건 조회(활성만) + 최소 컬럼 로딩
        """
        q = (
    ...
    async def get_many(self, vessel_ids: List[int]) -> List[VesselModel]:
        """
        여러 건 조회 (활성만)
    ...
    async def get_paginated(self, request: PaginateVesselRequest) -> CursorPaginationResult[VesselResponseSchema]:
        """
    ...
    async def update(
        ...
        """
        if not payload:
            return await self.get(vessel_id)
    ...
    async def hard_delete_by_id(self, vessel_id: int) -> None:
        """단건 하드 삭제"""
