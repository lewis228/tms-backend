...
class VesselService:
    ...
    def __init__(self, db: AsyncSession, tenant_id: int):
        self.db = db
        self.repo = VesselRepository(db, tenant_id)
    ...
    async def create(
        ...
        return VesselResponseSchema.model_validate(row)
    ...
    async def create_bulk(
        self,
        payload: VesselBulkCreateRequest,
        actor_user_id: int | None = None,
    ) -> VesselBulkCreateResponseSchema:
        """
        ...
        for item in payload.items:
            ...
            vessel = VesselResponseSchema.model_validate(row)
        ...
        return VesselBulkCreateResponseSchema(
            results=results,
            summary=BulkSummary(
    ...
    async def get(self, vessel_id: int) -> VesselResponseSchema:
        row = await self.repo.get(vessel_id)
        if not row:
            raise NotFoundException("거래처")
        return VesselResponseSchema.model_validate(row)

    async def list_paginated(
        self, request: PaginateVesselRequest
    ) -> CursorPaginationResult[VesselResponseSchema]:
        """
        커서 기반 페이지네이션:
          - meta.count / meta.hasMore / data(VesselResponseSchema[])
        """
        result = await self.repo.get_paginated(request)
        result.data = [VesselResponseSchema.model_validate(r) for r in result.data]
        return result
    ...
    async def sync_delta(self, since_str: str):
        ...
        result.items = [
            VesselResponseSchema.model_validate(r)
    ...
    async def update_bulk(
        self,
        payload: VesselBulkUpdateRequest,
        actor_user_id: int | None = None,
    ) -> VesselBulkUpdateResponseSchema:
        """
        ...
        # 여기까지 오면 전부 성공
        return VesselBulkUpdateResponseSchema(
            results=results,
            summary=BulkSummary(
                total=len(payload.items),
                succeeded=len(results),
    ...
    async def delete(
        ...
        return VesselDeleteResponseSchema(
            id=vessel_id,
            deleted=True,
            soft_deleted=True,
        )
    ...
    async def delete_bulk(
        self,
        payload: VesselBulkDeleteRequest,
        actor_user_id: int | None = None,
    ) -> VesselBulkDeleteResponseSchema:
        """
        거래처 벌크 삭제 (항상 소프트 삭제)
        ...
        return VesselBulkDeleteResponseSchema(
            results=results,
            summary=BulkSummary(
