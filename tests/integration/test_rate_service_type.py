...
"""컨플루언스 'Leg 전체 유형': 같은 From→To·Move 라도 Service Type 별 요율이 다르다.

같은 (point, zone, LOAD, 40ft) 셀이라도 service_type=LIVE 와 DROP 의 금액이 달라야 한다.
"""
...
async def test_service_type_differentiates_rate(db_session):
    ...
    point = RatePointModel(team_id=team.id, name="LA Terminal", point_type=PointType.TERMINAL)
    zone = RateZoneModel(team_id=team.id, name="SoCal")
    db_session.add_all([group, point, zone])
    await db_session.flush()

    db_session.add(RateZoneMemberModel(team_id=team.id, zone_id=zone.id, zip_code="90210"))
    ...
    # 같은 (LOAD, point) 슬롯, service_type 만 다른 두 시트
    sheet_live = RateSheetModel(team_id=team.id, rate_group_id=group.id, kind=SheetKind.POINT_ZONE,
                                move_type=RateMoveType.LOAD, service_type=RateServiceType.LIVE, row_point_id=point.id)
    sheet_drop = RateSheetModel(team_id=team.id, rate_group_id=group.id, kind=SheetKind.POINT_ZONE,
                                move_type=RateMoveType.LOAD, service_type=RateServiceType.DROP, row_point_id=point.id)
    db_session.add_all([sheet_live, sheet_drop])
    await db_session.flush()

    db_session.add(RateEntryModel(team_id=team.id, rate_sheet_id=sheet_live.id, col_zone_id=zone.id,
                                  container_size=RateContainerSize.SIZE_40, amount=Decimal("1000"),
                                  effective_from=date(2026, 1, 1)))
    db_session.add(RateEntryModel(team_id=team.id, rate_sheet_id=sheet_drop.id, col_zone_id=zone.id,
    ...
    common = dict(driver_id=driver.id, work_date=date(2026, 5, 9), move_type=RateMoveType.LOAD,
                  row_point_id=point.id, dest_zip="90210", container_size=RateContainerSize.SIZE_40)
